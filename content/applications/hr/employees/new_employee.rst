...
- :guilabel:`Schedule`: Select the :guilabel:`Working Hours` (required) and :guilabel:`Timezone` for
  ...
  working hours. Working hours can be modified or deleted here. Click :guilabel:`Save` to
  save any changes.
...
- :guilabel:`Private Contact`: Enter the personal :guilabel:`Address` for the employee. The
  ...
  :guilabel:`Save`. Some other information in the :guilabel:`Private Contact` section may
  auto-populate, if the address is already listed in the drop-down menu. Select the employee's
  preferred :guilabel:`Language` from the drop-down menu. Enter the employee's :guilabel:`Bank
  Account Number` using the drop-down menu. If the bank is not already configured (the typical
  situation when creating a new employee) enter the bank account number, and click
  :guilabel:`Create and Edit`. A :guilabel:`Create: Bank Account Number` for appears. Fill in the
  information, then click :guilabel:`Save`. Finally, enter the :guilabel:`Home-Work Distance` in the
  field. This field is only necessary if the employee is receiving any type of commuter benefits.
...
- :guilabel:`Previous Employer`: This section appears ony for Belgian companies, and will not be
...
- :guilabel:`Application Settings`: If applicable, enter the :guilabel:`Fleet Mobility Card` number.
  ...
  working at a work center. This value affects the manufacturing costs for a product, if the value
  of the manufactured product is not a fixed amount.
